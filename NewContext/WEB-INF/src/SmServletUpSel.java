...
pw.println("<script language=javascript>function check(){for(var c=0;c<document.input.elements.length;c++){var e=document.input.elements[c];if(e.value==\"\"){alert(\"모든 값을 입력 하셔야 합니다. \");return false}}var a=input.writer.value;a=trim(a);pass=checkByteLen(a,21);if(!pass){alert(\"이름 7자이하\");input.writer.focus();return false}var d=input.email.value;d=trim(d);pass=checkByteLen(d,30);if(!pass){alert(\"이메일 10자이하\");input.email.focus();return false}var b=input.subject.value;b=trim(b);pass=checkByteLen(b,30);if(!pass){alert(\"제목 10자이하\");input.subject.focus();return false}var f=input.content.value;f=trim(f);pass=checkByteLen(f,600);if(!pass){alert(\"글 200자이하\");input.writer.focus();return false}document.input.submit()};</script>");
